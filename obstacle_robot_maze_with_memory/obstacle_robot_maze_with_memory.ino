...
=======
//  SENSOR READ — INTERRUPT-DRIVEN (NEW)
//  ---------------------------------------------------------------------------
//  The original code measured each echo pulse with pulseIn(), which busy-
//  waits inside the Arduino core for up to ECHO_TIMEOUT_US (25ms) per call.
//
//  This version times the echo pulse with a hardware GPIO interrupt instead:
//    - trigger the sensor (same 10us TRIG pulse as before)
//    - an ISR on the ECHO pin (attached CHANGE) stamps micros() on the
//      rising edge and again on the falling edge, and computes the pulse
//      width itself -- entirely in interrupt context, no pulseIn() involved.
//    - the foreground code just polls a small "captured" flag (with the same
//      ECHO_TIMEOUT_US ceiling as a safety net) instead of blocking inside a
//      library call. The actual pulse-width measurement is now interrupt-
//      timestamped (lower jitter, not tied to how often the core happens to
//      poll), and this is what "interrupt-driven ultrasonic" means on a
//      microcontroller with no input-capture peripheral exposed to Arduino.
//
//  One HC-SR04 must still be triggered at a time -- firing all three
//  simultaneously lets them hear each other's echoes (crosstalk), which is a
//  physical/acoustic limitation, not something interrupts can fix.
// =============================================================================
enum SensorId { SENS_FRONT = 0, SENS_LEFT = 1, SENS_RIGHT = 2, SENS_COUNT = 3 };
...
static const UltrasonicChannel g_channel[SENS_COUNT] = {
  { TRIG_FRONT, ECHO_FRONT, "FRONT" },
  { TRIG_LEFT,  ECHO_LEFT,  "LEFT"  },
  { TRIG_RIGHT, ECHO_RIGHT, "RIGHT" },
};

// Everything an ISR touches must be volatile. Kept as small flat arrays
// (not structs) so each access is a single volatile load/store.
volatile uint32_t g_echoRiseUs[SENS_COUNT]     = {0, 0, 0};
volatile uint32_t g_echoDurationUs[SENS_COUNT] = {0, 0, 0};
volatile bool     g_echoCaptured[SENS_COUNT]   = {false, false, false};
volatile bool     g_echoArmed[SENS_COUNT]      = {false, false, false}; // true while a rising edge is expected/pending

// One ISR template instantiated per sensor -> three distinct IRAM function
// pointers (echoISR<0>, echoISR<1>, echoISR<2>), each hard-coded to its own
// index so there's no shared-parameter ambiguity between sensors. Must stay
// tiny and non-blocking: no Serial, no delay(), no heap use.
template <int N>
void IRAM_ATTR echoISR() {
  if (digitalRead(g_channel[N].echoPin) == HIGH) {
    g_echoRiseUs[N] = micros();
    g_echoArmed[N] = true;
  } else if (g_echoArmed[N]) {
    g_echoDurationUs[N] = micros() - g_echoRiseUs[N];
    g_echoCaptured[N] = true;
    g_echoArmed[N] = false;
  }
}

// NEW: call once from setup() after pinMode()s are configured.
void attachUltrasonicInterrupts() {
  attachInterrupt(digitalPinToInterrupt(ECHO_FRONT), echoISR<SENS_FRONT>, CHANGE);
  attachInterrupt(digitalPinToInterrupt(ECHO_LEFT),  echoISR<SENS_LEFT>,  CHANGE);
  attachInterrupt(digitalPinToInterrupt(ECHO_RIGHT), echoISR<SENS_RIGHT>, CHANGE);
  DBG_PRINTLN("[SENSOR] Echo-pin interrupts attached (CHANGE) for FRONT/LEFT/RIGHT");
}

// Fires one TRIG pulse and waits only on the *result flag* (not inside a
// library call) until the ISR reports a captured echo or ECHO_TIMEOUT_US
// elapses. Returns distance in cm; timeout/no-echo is reported as 400cm
// (far/clear), matching the original sensor's convention.
float readDistanceCmRaw(int id, const char* label) {
  noInterrupts();
  g_echoCaptured[id] = false;
  g_echoArmed[id] = false;
  interrupts();

  digitalWrite(g_channel[id].trigPin, LOW);
  delayMicroseconds(2);
  digitalWrite(g_channel[id].trigPin, HIGH);
  delayMicroseconds(10);           // only truly-unavoidable blocking left: the HC-SR04 trigger pulse itself
  digitalWrite(g_channel[id].trigPin, LOW);

  uint32_t waitStartUs = micros();
  while (!g_echoCaptured[id]) {
    if ((micros() - waitStartUs) > ECHO_TIMEOUT_US) {
      DBG_PRINTF("[SENSOR][%s] no echo within %luus (timeout) -> treating as 400cm (clear)\n",
                 label, ECHO_TIMEOUT_US);
      noInterrupts();
      g_echoArmed[id] = false;     // disarm so a late/stray edge after timeout can't corrupt the next reading
      interrupts();
      return 400.0f;
    }
  }
  ...
  noInterrupts();
  uint32_t duration = g_echoDurationUs[id];
  interrupts();

  float cm = duration * 0.01715f;                // convert us -> cm
  DBG_PRINTF("[SENSOR][%s] echo=%luus (ISR-timestamped) -> %.1fcm\n", label, (unsigned long)duration, cm);
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
  return cm;
}
...
=======
float readDistanceCm(int id, const char* label) {
  float s1 = readDistanceCmRaw(id, label); delayMicroseconds(300);
  float s2 = readDistanceCmRaw(id, label); delayMicroseconds(300);
  float s3 = readDistanceCmRaw(id, label);
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
...
=======
  s.F = readDistanceCm(SENS_FRONT, "FRONT");
  delay(10);
  s.L = readDistanceCm(SENS_LEFT,  "LEFT");
  delay(10);
  s.R = readDistanceCm(SENS_RIGHT, "RIGHT");
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
  delay(10);
...
=======
  float d0 = readDistanceCm(SENS_FRONT, "FRONT-SQ");
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
...
=======
  float d1 = readDistanceCm(SENS_FRONT, "FRONT-SQ");
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
...
=======
    float d = readDistanceCm(SENS_FRONT, "FRONT-SQ");
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
...
<<<<<<< HEAD
=======
  attachUltrasonicInterrupts();   // NEW: interrupt-driven echo timing (see SENSOR READ section)
>>>>>>> faa9f07 (obstacle_robot_maze_with_memory update)
